...
const DetailedVenuePage = () => {
  ...
  const location = useLocation() as {
    pathname: any; state: LocationState 
}; // ✅ Correct typing
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      {/* Venue image */}
      <img src={venue.media?.[0]?.url || "https://via.placeholder.com/600x400"} alt={venue.media?.[0]?.alt || "Venue image"} className="w-full h-96 object-cover rounded-lg" />
      ...
      <section className="mt-6">
        {isLoggedIn ? (
          <BookingForm venueId={venue.id} initialDate={selectedDate} />
        ) : (
          <div className="space-y-4">
            <p className="text-gray-600">Log in or register to book this venue</p>
            <div className="flex space-x-4">
              <Link to="/login" className="flex-1 px-4 py-2 border border-black text-black rounded hover:bg-gray-400 text-center">
                Log in
              </Link>
              <Link to="/register" className="flex-1 px-4 py-2 bg-black text-white rounded hover:bg-gray-700 text-center">
                Register
              </Link>
            </div>
          </div>
      ...
      {venue.location?.lat && venue.location?.lng && (
        <section className="mt-6 h-64 w-full rounded-lg overflow-hidden shadow-lg">
          <MapContainer center={[venue.location.lat, venue.location.lng]} zoom={14} scrollWheelZoom={false} style={{ height: "100%", width: "100%" }}>
